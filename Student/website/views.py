...
@views.route("/removecourse", methods=["POST"])
@login_required
def remove_course():
    remove_course = json.loads(request.data)
    userid = remove_course["userid"]

    existing = Startcourse.query.filter_by(userid=userid).first()

    if existing is None:
        flash("You haven't choosen any subject", "Error")
